...
- **Sync (repo → global)** — `sync::sync_to_global` copies to the canonical `~/.agents/skills/<name>` and symlinks into the detected agent dirs; `ledger` appends to `sync.toml`. **Remove** — `sync::remove_path` unlinks/deletes an installed skill. Both run via the pending-op pattern (queued in pure input handling, executed in `main`), with a footer status line and modal overlays (`ui::modal_view`/`modal_hitmap` share geometry between draw + click).
